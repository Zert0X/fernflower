	...
	MouseDrop_T(var/atom/movable/T, mob/user)
		if (istype(T,/mob))
			...
			if(target == user && !user.stat && !user.weakened && !user.stunned && !user.paralysis)	// if drop self, then climbed in										// must be awake, not stunned or whatever
				...
				message_admins("ATTACK: [user] ([user.ckey])(<A HREF='?src=%admin_ref%;adminplayerobservejump=\ref[user]'>JMP</A>) climbed into in a disposals unit.", 2)
				...
				message_admins("ATTACK: [user] ([user.ckey])(<A HREF='?src=%admin_ref%;adminplayerobservejump=\ref[user]'>JMP</A>) placed [target] ([target.ckey]) in a disposals unit.", 2)
