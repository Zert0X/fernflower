...
/obj/structure/altar/hand_p(mob/user as mob)
	return src.attack_paw(user)
	return


/obj/structure/altar/attack_paw(mob/user as mob)
	if ((HULK in usr.mutations))
		usr << "\blue You destroy the altar."
		for(var/mob/O in oviewers())
			if ((O.client && !( O.blinded )))
				O << "\red [user] smashes the altar apart!"
		if(istype(src, /obj/structure/altar/reinforced))
			new /obj/item/weapon/table_parts/reinforced( src.loc )
		else if(istype(src, /obj/structure/altar/woodentable))
			new/obj/item/weapon/table_parts/wood( src.loc )
		else
			new /obj/item/weapon/table_parts( src.loc )
		src.density = 0
		del(src)
	if (!( locate(/obj/structure/altar, user.loc) ))
		step(user, get_dir(user, src))
		if (user.loc == src.loc)
			user.layer = TURF_LAYER
			for(var/mob/O in oviewers())
				if ((O.client && !( O.blinded )))
					O << "[user] hides under the altar!"
				//Foreach goto(69)
	return


/obj/structure/altar/attack_alien(mob/user as mob) //Removed code for larva since it doesn't work. Previous code is now a larva ability. /N
	usr << "\green You destroy the altar."
	for(var/mob/O in oviewers())
		if ((O.client && !( O.blinded )))
			O << "\red [user] slices the table apart!"
	if(istype(src, /obj/structure/altar/reinforced))
...
/obj/structure/altar/attack_animal(mob/living/simple_animal/user as mob) //Removed code for larva since it doesn't work. Previous code is now a larva ability. /N
	if(user.wall_smash)
		usr << "\red You destroy the altar."
		for(var/mob/O in oviewers())
			if ((O.client && !( O.blinded )))
				O << "\red [user] smashes the altar apart!"
		if(istype(src, /obj/structure/altar/reinforced))
...
/obj/structure/altar/attack_hand(mob/user as mob)
	if ((HULK in usr.mutations) || (SUPRSTR in usr.augmentations))
		usr << "\blue You destroy the altar."
		for(var/mob/O in oviewers())
			if ((O.client && !( O.blinded )))
...
/obj/structure/altar/CanPass(atom/movable/mover, turf/target, height=0, air_group=0)
	if(air_group || (height==0)) return 1
...
/obj/structure/altar/MouseDrop_T(obj/O as obj, mob/user as mob)
...
/obj/structure/altar/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if (istype(W, /obj/item/weapon/grab) && get_dist(src,user)<2)
		var/obj/item/weapon/grab/G = W
		if(G.state<2)
			if(ishuman(G.affecting))
