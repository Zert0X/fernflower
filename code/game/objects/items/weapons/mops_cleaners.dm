...
/obj/item/weapon/pepperspray/elited //And this is for personal defense.
...
/obj/item/weapon/pepperspray/elited/New()
	var/datum/reagents/R = new/datum/reagents(ReagentAmount)
	reagents = R
	R.my_atom = src
	R.add_reagent("condensedcapsaicin", ReagentAmount)

/obj/item/weapon/pepperspray/elited/attack(mob/living/carbon/human/M as mob, mob/user as mob)
	return
...
/obj/item/weapon/pepperspray/elited/examine()
	set src in usr
	if(BottleSize)
		usr << text("\icon[] [] units of pepperspray left!", src, src.reagents.total_volume)
		..()
